...
public class UserStorage extends Storage<User> {
    ...
    public UserStorage(String storageDirPath, String relativeFilePath) throws IOException, IllegalArgumentException, NullPointerException {
        // calling parent constructor
        super(storageDirPath, relativeFilePath);

        // setting counter file path
        this.counterFilePath = config.getStoragePath()+config.getUserCounterPath();

        // loading users data from storage file
        this.loadData();

        // initializing the User global counter
        this.initCounter();
    }


    // used to initialize the User global counter
    private void initCounter() {
        ...
        try(FileReader reader = new FileReader(this.counterFilePath)) {
            ...
            if(Objects.nonNull(counter)) User.setCounter(counter.getCounter());
    ...
    private void loadData() {
        ...
        try(FileReader reader = new FileReader(this.storageFilePath)) {
            JsonReader jsonReader = new JsonReader(reader);
            jsonReader.setLenient(true);
            Type storageType = new TypeToken<ConcurrentHashMap<String, User>>(){}.getType();
    ...
    private void updateCounterFile() {
        ...
        try(FileWriter writer = new FileWriter(this.counterFilePath)) {
            ...
            counter.setCounter(User.incrementCounter());
